...
Updated by Bryan to have more functionality (bar chart)
"""
...
def plot(series, cfg=None):
    ...
    `tips` specifies a cutoff (greater or equal) where only tips at printed, 
    not vertical bars
    """
    if len(series) == 0:
        return ''

    if not isinstance(series[0], list) and not isinstance(series[0],np.ndarray):
    ...
    linestyle = cfg.get('linestyle','line')

    colors = cfg.get('colors', [None])
    tips = cfg.get('tips', 1000) # arbitrarily large number to always plot lines
    ...
    if linestyle == 'line':
        for i in range(0, len(series)):
            ...
            for x in range(0, len(series[i]) - 1):
                ...
                if i < tips: # only plot height for first two points
                    start = min(y0, y1) + 1
                    end = max(y0, y1)
        ...
        '''
        in a bar chart, we have a list of data series
        we take a column from each data series, assuming they are the same shape
        and aligned with the same x-ticks
        sort them, and then make an ascii bar chart plot. sorting makes sure 
        all of the bars are visible by plotting the values from smallest to largest
        '''
        symbols = [BOLD + '│' + reset]
        for i in range(len(series[0])):
            ys = [y[i] for y in series]
            ind = sorted(range(len(ys)),key=ys.__getitem__,reverse=True)
            yss = [ys[i] for i in ind]
            
            for ix,y in enumerate(yss):
                if isnan(y):
                    continue
                y0 = scaled(y)
                result[rows - y0][i + offset] = colored(symbols[0], colors[ind[ix]])
                
                start = min(y0, scaled(0))
                end = max(y0, scaled(0))
                for y in range(start, end):
                    result[rows - y][i + offset] = colored(symbols[0], colors[ind[ix]])
